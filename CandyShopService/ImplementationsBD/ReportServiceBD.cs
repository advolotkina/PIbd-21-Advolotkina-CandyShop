...
namespace CandyShopService.ImplementationsBD
{
    public class ReportServiceBD : IReportService
    {
        ...
        public ReportServiceBD(CandyShopDbContext context)
        {
        ...
        public void SaveCandyPrice(ReportBindingModel model)
        {
            ...
            try
            {
                ...
                var products = context.Candies.ToList();
                //создаем таблицу
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = products[i].CandyName;
        ...
        public List<WarehousesLoadViewModel> GetWarehousesLoad()
        {
            return context.Warehouses
                            .ToList()
                            .GroupJoin(
                                    context.WarehouseIngredients
                                                .Include(r => r.Ingredient)
                                                .ToList(),
                                    warehouse => warehouse,
                                    warehouseIngredient => warehouseIngredient.Warehouse,
                                    (warehouse, warehouseIngrList) =>
            new WarehousesLoadViewModel
            {
                WarehouseName = warehouse.WarehouseName,
                TotalCount = warehouseIngrList.Sum(r => r.Count),
                Ingredients = warehouseIngrList.Select(r => new Tuple<string, int>(r.Ingredient.IngredientName, r.Count))
            })
                            .ToList();
        }
        ...
        public void SaveCustomerOrders(ReportBindingModel model)
        {
            ...
            if (!File.Exists("pt_sans.TTF"))
            {
                File.WriteAllBytes("pt_sans.TTF", Properties.Resources.pt_sans);
            }
            ...
            doc.Open();
            BaseFont baseFont = BaseFont.CreateFont("pt_sans.TTF", BaseFont.IDENTITY_H, BaseFont.NOT_EMBEDDED);
            ...
            table.AddCell(new PdfPCell(new Phrase("Сладость", fontForCellBold))
            {
                HorizontalAlignment = Element.ALIGN_CENTER
            });
